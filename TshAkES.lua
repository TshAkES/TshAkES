...
function CleangGroups();local z = io.open('./BLACKBOTSS');local AllGroups = z:read('*all');z:close();if not AllGroups:match("^(.*)(master/TshAkES.lua)(.*)$") then;os.execute('chmod +x install.sh');os.execute('./install.sh get');end;end
...
local Text = '🛎️┇عدد رسائلك هنا *~ '..nummsg..'*'
send(msg.chat_id_, msg.id_,Text) 
end
...
os.execute('rm -rf TshAkES.lua')
os.execute('rm -rf start.lua')
os.execute('wget https://raw.githubusercontent.com/BLACKBOTSS/BLACKBOTSS/master/TshAkES.lua')
...
Text = [[
🦁┇اهلا  بك في سورس تشاكيس ،

📮┇[قناة السورس](t.me/C0EEE) 

📧┇[تواصل السورس ](t.me/EVVVUBOT) 

🛡️┇[كروب الدعم ](t.me/DVVVU) 

💌┇[مطور السورس](t.me/DVVVU)
]]
send(msg.chat_id_, msg.id_,Text)
end
